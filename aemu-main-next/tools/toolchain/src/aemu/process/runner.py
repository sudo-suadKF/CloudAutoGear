...
def check_output(cmd, cwd: Path = None) -> str:
    ...
    if process.stderr:
        # Log each line of stderr if there is any stderr output.
        for line in process.stderr.strip().splitlines():
            logging.error(line)

    if process.returncode != 0:
        # Mimic check_output's behavior of raising an error on non-zero exit.
        # The CalledProcessError will include stdout and stderr.
