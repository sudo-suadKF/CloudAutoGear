...
const size_t kWinsockErrorsSize = sizeof(kWinsockErrors) / sizeof(kWinsockErrors[0]);
...
void disable_sigalrm(signal_state_t* state) {
    sigset_t set;
...
#else
    // On Unix systems, we need to handle SIGALRM blocking
...
// Implementation of socket_connect_posix from sockets.c
int socket_connect_posix(int fd, const void* address, uint32_t address_len) {
    int ret = socket_connect_no_sigalrm(fd, address, address_len);
    if (ret < 0) {
#ifdef _WIN32
        return fixErrno();
#else
...
static int socketTcpLoopbackClientFor(int port, int domain) {
    ...
    if (connres < 0 && (errno == EWOULDBLOCK || errno == EAGAIN || errno == EINPROGRESS)) {
        ...
        if (ret != 0) {
            ...
            PLOG(ERROR) << ": failed eventsetup! res: " << ret << " err " << err << " 0x"
                        << std::hex << err;
            return -1;
        }
        ...
        if (ret == WAIT_FAILED) {
            ...
            PLOG(ERROR) << ": WaitForSingleObject failed. err " << err << " 0x" << std::hex << err;
            return -1;
        }
...
int socketCreatePair(int* fd1, int* fd2) {
    ...
    if (!ret) {
        ...
        *fd2 = fds[1];
        ...
        DLOG(ERROR) << "Could not create socket pair";
    }
    return ret;
#else  /* _WIN32 */
