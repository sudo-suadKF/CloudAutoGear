...
- **Response Body (JSON)**:
  ```json
  {
    ...
    "hardwareConfig": {           // [DEPRECATED] Key-value pairs of hardware config
      "hw.lcd.width": "1080",
      "hw.lcd.height": "1920",
      ...
    },
    "platformConfig": {           // Preferred: Key-value pairs of hardware/platform config
      "hw.lcd.width": "1080",
      "hw.lcd.height": "1920",
      ...
    },
  ...
  ```
  _Note: The frontend specifically expects `platformConfig["hw.lcd.width"]` (or `hardwareConfig["hw.lcd.width"]`) and
  `platformConfig["hw.lcd.height"]` (or `hardwareConfig["hw.lcd.height"]`) to scale mouse/touch coordinates._
...
- **Flow**:
  ...
  1. **Start Signal (Server -> Client)**: Server immediately sends the ICE
     Servers configuration.
  1. **SDP Offer (Server -> Client)**: Server sends the WebRTC SDP Offer.
  1. **SDP Answer (Client -> Server)**: Client responds with the WebRTC SDP
     Answer.
  1. **Trickle ICE (Bidirectional)**: Server and Client exchange ICE candidates
     as they are gathered.
  1. **Teardown**: Either side sends `bye` before closing.
...
1. The client sends these binary bytes over the `"input"` data channel.
1. The gateway server/video bridge **must receive these binary messages** from the WebRTC data channel and forward them directly to the emulator's corresponding gRPC streaming method:
